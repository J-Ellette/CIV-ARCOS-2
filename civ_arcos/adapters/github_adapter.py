...
class GitHubCollector(EvidenceCollector):
    ...
    def collect(self, repo_url: str = "", token: Optional[str] = None, **kwargs) -> List[Evidence]:  # type: ignore[override]
        ...
        if isinstance(commits, list):
            commit_items = cast(List[Dict[str, Any]], commits)
            evidence_list.append(
                self._make_evidence(
                    "commit_history",
                    repo_url,
                    {"commits": commit_items[:10]},
                    {
    ...
    def collect_from_github(self, repo_url: str, commit_hash: str) -> List[Evidence]:
        ...
        if not owner or not repo:
            return []
        data = (
            self._get(f"{self.BASE_URL}/repos/{owner}/{repo}/commits/{commit_hash}")
            or {}
        )
        return [
            self._make_evidence(
                "commit_detail", repo_url, data, {"commit_hash": commit_hash}
            )
        ]
    ...
    def _make_evidence(
        self, etype: str, source: str, data: Dict[str, Any], provenance: Dict[str, Any]
    ) -> Evidence:
